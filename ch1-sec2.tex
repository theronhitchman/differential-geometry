...
\[
\left( T_{\alpha} \cdot T_{\beta} \right)' = T_{\alpha} (\kappa_{\beta}N_{\beta}) 
...
and there is another constant $c$ so that $\tau = a\kappa$. We may now find 
...
|1-\varepsilon\kappa_{\alpha} |$. If the tangent vectors are parallel, we must 
...
\dfrac{d\beta}{ds} & = T_{\alpha} 
 + \varepsilon\left( -\sin(\theta) \theta' N + \cos(\theta) (-\kappa T + \tau B) 
  + \cos(\theta) \theta' B + \sin(\theta) ( -\tau N) \right) \\
& = (1-\varepsilon\kappa\cos(\theta)) T - \varepsilon\sin(\theta) (\theta' + \tau) N 
 + \varepsilon\cos(\theta) ( \tau + \theta') B .
\end{split}
\]
...
\beta(s) = \left( a \cos(s/c) +\varepsilon\left[ -\cos(bs/c^2)\cos(s/c) 
  - \dfrac{b}{c}\sin(bs/c^2)\sin(s/c)\right] ,
	a \sin(s/c) +\varepsilon \left[ \cos(bs/c^2)\sin(s/c) 
	+ \dfrac{b}{c}\sin(bs/c^2)\cos(s/c)\right] ,
